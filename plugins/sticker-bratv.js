...
let texto1 = packstickers.text1 || MARCA // <- TU MARCA POR DEFECTO
let texto2 = packstickers.text2 || MARCA // <- TU MARCA POR DEFECTO
switch (command) {
case 'brat': {
...
if (frase.length > 30) return await m.react('✖️'), conn.reply(m.chat, `ꕥ El texto no puede tener más de 30 caracteres.\n${MARCA}`, m) // <- TU MARCA
await m.react('🕒')
const quoteObj = { type: 'quote', format: 'png', backgroundColor: '#000', width: 512, height: 768, scale: 2, messages: [{ entities: [], avatar: true, from: { id: 1, name: nombre, photo: { url: pp } }, text: frase, replyMessage: {} }]}
