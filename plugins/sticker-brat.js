...
let texto1 = packstickers.text1 || global.packsticker || '𝗗𝗢𝗟𝗜𝗘 𝗕𝗢𝗧'
let texto2 = packstickers.text2 || global.packsticker2 || '✨ Premium Edition ✨'

switch (command) {
case 'brat': {
text = m.quoted?.text || text
if (!text) return conn.sendMessage(m.chat, { text: '🎀 Por favor responde a un mensaje o escribe un texto para crear el sticker.' }, { quoted: m })
await m.react('⏳')
...
if (!textFinal) return conn.reply(m.chat, `🎀 Escribe un texto para crear el quote sticker.`, m)
...
case 'take': case 'wm': {
if (!m.quoted) return m.reply(`🎀 Responde a un sticker con *${usedPrefix + command}* + nuevo nombre.\n> Ejemplo: *${usedPrefix + command}* Dollie Bot | By Dollie`)
await m.react('⏳')
const stickerData = await m.quoted.download()
if (!stickerData) return await m.react('❌'), m.reply('ꕥ No se pudo descargar el sticker.')
...
await m.react('❌')
conn.sendMessage(m.chat, { text: `⚠️ Ocurrió un problemita.\n> Usa *${usedPrefix}report* para reportarlo.\n\n${e.message}` }, { quoted: m })
}}

handler.tags = ['sticker']
handler.help = ['brat', 'bratv']
